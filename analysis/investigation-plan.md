...
An informal follow-up of approximately five CPU-versus-CPU runs comparing a
gravity-edited build with the original suggested that roughly 30% of launches
involving early hyperspace showed the effect. Treat this as a reproduction clue,
not a measured rate: exact build hashes, per-build counts, hyperspace-event
counts, and event-by-event outcomes were not retained. The apparent early-
hyperspace association raises the priority of the entry dirty/XOR transition.
...
- Implemented a size-preserving `EDIT-HYPER-02` prototype from the exact original. A guarded near call replaces the existing round-start pause clear; a nine-byte internal helper replays that clear and zeros `DS:0060/0061` before initialization. Static size, MZ, checksum, ownership, and disassembly validation passed.
- Completed the first controlled `EDIT-HYPER-02` runtime regression. A stop immediately after the helper proved both counters zero before original round initialization; timed left and right hyperspace/F1/F2 sequences produced neither inherited particles nor a default-position ghost after movement, and an ordinary hyperspace cycle still completed for each side. Delayed restarts, simultaneous effects, option combinations, and a bounded repeated-trace pilot remain open.
